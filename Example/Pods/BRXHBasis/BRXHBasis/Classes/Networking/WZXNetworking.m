...
static NSString *md5TimeStr = nil;
...
+ (WZXURLSessionTask *)getWithUrl:(NSString *)url params:(NSDictionary *)params success:(WZXResponseSuccess)success fail:(WZXResponseError)fail
{
    NSString *newUrl = nil;
    if ([url containsString:@"?"]) {//已含有参数，直接后面拼接
        newUrl = [NSString stringWithFormat:@"%@&app=%@",url,AFAPPType];
    }else{
        newUrl = [NSString stringWithFormat:@"%@?app=%@",url,AFAPPType];
    }
   
    
    return [self WZX_requestWithUrl:newUrl refreshCache:NO httpMedth:1 params:params success:success fail:fail];
}
...
+ (WZXURLSessionTask *)postWithUrl:(NSString *)url params:(NSDictionary *)params success:(WZXResponseSuccess)success fail:(WZXResponseError)fail
{
    ...
    long timeString =[dat timeIntervalSince1970];
    
    timeStr = [NSString stringWithFormat:@"%ld", timeString];//转为字符型
    md5TimeStr =  [NSString md5HexDigest:[[NSString md5HexDigest:[timeStr stringByAppendingString:@"12345"]] stringByAppendingString:@"54321"]];
    ...
    return  [self WZX_requestWithUrl:newUrl refreshCache:NO httpMedth:2 params:params success:success fail:fail];
}
...
+ (WZXURLSessionTask *)WZX_requestWithUrl:(NSString *)url refreshCache:(BOOL)refreshCache httpMedth:(NSUInteger)httpMethod params:(NSDictionary *)params success:(WZXResponseSuccess)success fail:(WZXResponseError)fail{
    ...
    } else if (httpMethod == 2)
    {
        ...
        NSString *timeStr = [zx_httpHeaders objectForKey:@"ONE"];
        if (md5TimeStr.length>0) {
            if (timeStr) {
                
                if (!params) {
                    params = @{@"check":md5TimeStr};
                }else{
                    NSMutableDictionary *mutableDic = [NSMutableDictionary dictionaryWithDictionary:params];
                    [mutableDic setValue:md5TimeStr forKey:@"check"];
                    params = [NSMutableDictionary dictionaryWithDictionary:mutableDic];
                }
            }
...
+ (AFHTTPSessionManager *)manager{
    ...
    // 后台接口验证
        if (!zx_httpHeaders) {
            ...
            zx_httpHeaders = @{@"lang":Klang};
        }else{
            NSMutableDictionary *mutableDic = [NSMutableDictionary dictionaryWithDictionary:zx_httpHeaders];
            [mutableDic setObject:Klang forKey:@"lang"];
            zx_httpHeaders = [NSDictionary dictionaryWithDictionary:mutableDic];
        }
    ...
    if (zx_responseType!=kWZXResponseTypeData) {
...
+ (void)test{
    NSString *url = @"http://c.zhcw.com/zhcwapp/serv.do";
    NSDictionary *parametersDictionary = @{@"transMessage":@"{\"message\":{\"head\":{\"deviceId\":\"8F0CA1D8-7E7A-4174-ADF7-B1CB9E516A37\",\"ua\":\"iPhone -iOS10.200000\",\"src\":\"0003000001|0401005010\",\"messageID\":\"9999201711211536499\",\"timeStamp\":\"20171121153649\",\"transactionType\":\"300203\",\"messengerID\":\"9999\",\"imei\":\"1\",\"sysType\":\"Z\"},\"body\":{\"pageSize\":\"20\",\"pageNo\":\"1\"}}}"};
